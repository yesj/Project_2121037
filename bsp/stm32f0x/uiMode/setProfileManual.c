...
static void	F_setProfileLevelKey(rt_uint8_t keyCode,rt_bool_t LongKeyStartFlg)
{
		switch(keyCode)
		{
			...
			if(F_NumberUp_8(&set_profile_data.menualLevel,1,noCycleNumberVal).complyFlg == YesComplyVal) {
				F_Menual_LevelCount(set_profile_data.menualLevel.number,sport_data.progfileArry,ProgfileDataSizeVal);
			...
			if(F_NumberDown_8(&set_profile_data.menualLevel,1,noCycleNumberVal).complyFlg == YesComplyVal) {
				F_Menual_LevelCount(set_profile_data.menualLevel.number,sport_data.progfileArry,ProgfileDataSizeVal);
				if(LongKeyStartFlg == 0){
					bz_short();
				}
...
static void	F_setProfileWorkOutTImeKey(rt_uint8_t keyCode,rt_bool_t LongKeyStartFlg)
{
		switch(keyCode)
		{
			...
			F_NumberUp_8(&set_profile_data.menualWorkoutTime,1,cycleNumberVal);
				break;
			case	resistance_down_KeyVal:
			if(LongKeyStartFlg == 0){
				bz_short();
			}
			F_NumberDown_8(&set_profile_data.menualWorkoutTime,1,cycleNumberVal);
				break;
		}
}

void F_setProfileManual(void)
{
					...
					RT_WAITING_FOREVER, &e) == RT_EOK)
			{
				...
				if((e & time_100ms_val) == time_100ms_val)
				{
					...
					if(ui_action.TemporarySeatPositionEvent == TemporarySeatPositionNormalEventVal) {
						...
						{
							case	setProfileLevelEventVal:
							F_showProfileSetLeve(set_profile_data.menualLevel.number);
							F_ShowProgfileGraph(sport_data.progfileArry,ProgfileDataSizeVal,CommonModeVal,0,0);
								break;
							case	setProfileWorkOutTImeEventVal:
							SetWorkoutTime.timeH = set_profile_data.menualWorkoutTime.number;
...
void F_setProfileManualInit(void)
{
	ui_action.Status = setProfileManualVal;
	ui_action.Event = setProfileLevelEventVal;
	F_Menual_LevelCount(set_profile_data.menualLevel.number,sport_data.progfileArry,ProgfileDataSizeVal);
}
