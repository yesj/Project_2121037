...
static void	F_setProfileLevelKey(rt_uint8_t keyCode)
{
		switch(keyCode)
		{
			...
			bz_short();
			F_ProfileSportInit(setInterval_1_EventVal);
				break;
			case	enter_KeyVal:
			bz_short();
			...
			bz_short();
			F_setProfileChooseInit(setInterval_1_EventVal);
				break;
			case	resistance_up_KeyVal:
			if(F_NumberUp_8(&set_profile_data.interval_1_Level,1,noCycleNumberVal).complyFlg == YesComplyVal) {
				F_Interval_1_LevelCount(set_profile_data.interval_1_Level.number,sport_data.progfileArry,ProgfileDataSizeVal);
				bz_short();
			}
				break;
			case	resistance_down_KeyVal:
			if(F_NumberDown_8(&set_profile_data.interval_1_Level,1,noCycleNumberVal).complyFlg == YesComplyVal) {
				F_Interval_1_LevelCount(set_profile_data.interval_1_Level.number,sport_data.progfileArry,ProgfileDataSizeVal);
				bz_short();
			}
				break;
...
static void	F_setProfileWorkOutTImeKey(rt_uint8_t keyCode)
{
		switch(keyCode)
		{
			...
			case	resistance_up_KeyVal:
			bz_short();
			F_NumberUp_8(&set_profile_data.interval_1_WorkoutMinTime,1,cycleNumberVal);
				break;
			case	resistance_down_KeyVal:
			bz_short();
			F_NumberDown_8(&set_profile_data.interval_1_WorkoutMinTime,1,cycleNumberVal);
				break;
		}
}


void F_setProfileInterval_1(void)
{
...
void F_setProfileInterval_1Init(void)
{
	ui_action.Status = setInterval_1_Val;
	ui_action.Event = setProfileLevelEventVal;
	F_Interval_1_LevelCount(set_profile_data.interval_1_Level.number,sport_data.progfileArry,ProgfileDataSizeVal);
}
